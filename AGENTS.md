...
项目采用“人工目标 -> Agent A 设计提示词 -> Agent B 实现测试 -> Agent C 验收并更新核心逻辑文档 -> 通过后按版本号 git 提交 -> 人工复核 -> 下一轮”的循环。
...
- 如果验收最终通过：必须按本轮版本号自动创建 git commit，提交范围只包含本轮相关文件。
...
- 提交成功后，最终汇报必须包含 commit hash、版本号、提交说明、核心改动、测试命令和结果。
- 验收不通过、测试失败、文档未同步、版本号不明确或存在无关 diff 时，Agent C 不得提交。
