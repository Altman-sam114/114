...
该脚本会核对 manifest 的分支/提交/run/attempt/关键路径、`ci-run-context.txt` 的身份字段和 artifact 名称、artifact index 的必需路径和本地文件/目录非空状态、JUnit 四个阶段 testcase、failure summary 的日志入口、Mac/iOS build 成功标记和 Mac 快照 manifest。
