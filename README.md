...
- 计时页的当前日程也可按分类筛选，选中分类后显示当前筛选摘要、可启动待办数和清除入口，摘要和空分类清除入口具备 44pt 点击区、分类名可访问标签和 Voice Control 输入标签，任务行会显示带可访问标签和 Voice Control 分类名输入标签的分类 badge，方便从待办直接进入专注。
...
- macOS 版提供状态栏剩余时间、弹出式极简计时器、带分类和时间上下文的当前待办、可直达日程/统计/设置的快捷面板、详细功能窗口、任务行常驻分类 badge、分类筛选快速新增预填提示、连续快速新增时保留刚使用的分类、任务行和小窗分类 badge 支持分类名 Voice Control 输入标签并优先使用预设色、带分类名可访问语义和稳定点击区的筛选摘要新增/清除按钮和筛选/总数计数、桌面通知、触觉反馈、Pro 统计和 Mac 日历同步。
...
该脚本会核对 manifest 的分支/提交/run/attempt/short SHA、workflow/project/scheme/destination 元数据、createdAt、关键路径、project reports allowlist，`ci-run-context.txt` 的身份字段和 artifact 名称，artifact index 的身份字段、version、createdAt、必需路径、本地文件/目录非空状态、required entry 的本地 byteCount/fileCount/recursiveByteCount 复算和 totals/entries 一致性，artifact 根目录/报告目录/快照目录不存在未声明额外文件，JUnit 四个阶段 testcase、JUnit outcome 与 manifest outcome，failure summary 的身份字段、阶段 outcome 和日志入口，static-checks 日志 marker、Xcode 版本日志、分类摘要动作 contract marker、分类 chip 可访问 contract marker、Mac/iOS build 成功标记、Mac 快照 manifest 以及 manifest 中每张 PNG 的 byteCount 与下载文件大小一致；`verify_project.sh` 还用分类摘要 marker 缺失、错误 JUnit outcome、错误 artifact 名称、错误 manifest 元数据、错误 artifact index 身份、错误 artifact index totals、额外 artifact 文件、本地文件大小篡改、本地缺失产物和快照 manifest 大小篡改负向 fixture 确认 validator 不会放行错包、混入包或残缺下载。
