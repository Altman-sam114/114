...
- 日程页改为日历/待办样式，可按日、周、月和分类查看；分类摘要可接力到计时并选择首个可启动待办，任务行可精确设为当前计时待办，两者都不会自动开始。日期格会读出日期、待办数、已选中和非本月状态，并支持日期 Voice Control 输入标签；待办行会显示支持分类名 Voice Control 输入标签的分类 badge，完成/启用/编辑/删除操作会读出任务名和分类，并支持任务名或分类名 Voice Control 输入标签，也可启用/停用、循环、到时间自动开启番茄钟。
