...
- 当前云端 CI 结果包覆盖静态检查、项目验证、`ChronoFocusMac` build、`ChronoFocus` iOS generic build、artifact index、index totals 一致性、run context、JUnit outcome、failure summary 身份/outcome、static-checks 日志 marker、Xcode 版本日志、分类可访问 contract marker、Mac 快照 manifest 和失败阶段关键错误摘录。
