...
- v1.1 起，已有分类使用量必须按同一 POSIX folding key 统计全部 `store.tasks`；正数显示任务数量，零值显示“历史”，不得按数量重排、持久化或改变 v1.0 的代表色/草稿规则。VoiceOver 分别读出“<分类>分类，<N>项任务”和“<分类>分类，仅历史专注记录”，选中 trait、Voice Control、iOS 44pt、动态字体和 Mac 静态/真实共用 option 必须保留。目标 marker 为 `Existing category usage context contracts verified.`，validator 目标结果为 `PASS verify_project existing category usage context contracts`；当前云端证据为 `pending`。
