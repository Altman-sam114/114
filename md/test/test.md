...
- v1.3 的云端项目验证必须输出 `Schedule to timer handoff contracts verified.`，artifact validator 必须输出 `PASS verify_project schedule to timer handoff contracts`；marker 缺失负向 fixture 只允许该项 FAIL。完整预期基线为 `129 PASS / 0 FAIL`；`detail-schedule.png` 验收接力入口，`detail-timer.png` 验收产品分类筛选和目标任务 selected 终态。当前云端证据为 `pending`。
