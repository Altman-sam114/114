...
`--archive`、`--archive-size`、`--archive-digest` 必须全有或全无。完整参数组会针对同一个原始 ZIP 输出 `PASS artifact archive byte count`、`PASS artifact archive sha256 digest` 和 `PASS artifact archive zip integrity`；省略全部 archive 参数时，既有目录-only 调用保持兼容，只传完整 archive 参数时保持 v0.97 兼容。`--artifact-metadata` 只能与完整 archive 参数组共同提供，单独提供必须以参数错误退出。

Agent C 必须先从 GitHub API 获取最新 run 的原始响应，写入全新目录的 `artifacts-api.json.part`，成功且非空、最终文件不存在时才在同一文件系统原子改名。Metadata 必须是非空普通文件、拒绝 symlink 且不超过 `1_048_576` bytes，由 `JSON.parse` 结构化复判；不得复制进 artifact 解包目录。再使用同一 JSON 中的唯一 artifact id 下载 `<artifact-name>.zip.part`，启用失败即退出、跟随重定向和有限重试；在 `.part` 上核对 size、SHA-256 与 `unzip -t`，全部通过且最终 ZIP 不存在后才原子改名。任何既有目录、JSON、`.part`、最终 ZIP 或解包目录默认拒绝覆盖、删除或复用，失败证据必须保留。
