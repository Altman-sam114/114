...
- v1.2 起，已有分类达到 6 项时必须显示独立搜索、结果数/总数、清除和无结果状态；查询使用独立空字符串语义与 POSIX folding 名称子串匹配，只过滤稳定 option，不修改分类/颜色草稿或持久化。Mac 外部分类预填必须清除旧查询，真实/静态路径共享过滤结果。目标 marker 为 `Existing category search contracts verified.` 与对应 validator PASS；当前云端证据为 `pending`。
- v1.2 的 Run API 来源必须由 `event=push`、`actor.login=Altman-sam114`、`triggering_actor.login=Altman-sam114`、`head_repository.full_name=Altman-sam114/114` 四项独立 PASS，以及 `CI workflow run provenance contracts verified.` 和对应 validator PASS 证明；当前云端证据为 `pending`。
