...
该脚本会核对 manifest branch/commit/run/attempt 和关键路径字段、`ci-run-context.txt` 身份字段与 artifact 名称、artifact index 身份字段、必需路径与 kind、下载后本地文件/目录非空状态、JUnit 四个 testcase、failure summary 日志入口、Mac/iOS build 成功标记和 Mac 快照 manifest。`verify_project.sh` 用小型成功 fixture、错误 artifactName 负向 fixture、artifact index 身份错包负向 fixture 和本地缺失产物负向 fixture 覆盖 validator 的放行/拒绝路径。脚本只能辅助复判，不能替代 Agent C 对最新 `origin/main` run 和 artifact 来源的一致性核对。
...
- 检查分类预设、日程页和计时页分类筛选、新建预填、筛选摘要、iOS 日程筛选计数、计时页分类筛选摘要、分类摘要插入点和新增/清除动作接线、iOS 摘要快捷新增分类待办、Mac 待办筛选计数、Mac 快速新增分类预填提示、筛选优先级、44pt iOS 分类点击区域、iOS 设置页音色选择/试听、根视图非 Pro 音色清洗、Mac 小窗任务分类上下文、Mac 小窗直达详情入口、Mac 各详情页快照安全静态控件、CI iOS/错误摘录/artifact index/run context 结果包实现标记、结果包校验脚本语法、validator 小型成功、artifactName mismatch 负向、artifact index 身份错包负向和本地缺失产物负向 fixture、iOS simulator destination 解析 fixture。
