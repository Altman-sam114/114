...
- v0.94 起，iOS 计时页分类筛选无可启动待办时必须直接提供新增此分类和清除筛选动作，并打开预填当前分类的待办编辑 sheet；`verify_project.log` 必须包含 `Timer category empty state action contracts verified.`，Agent C artifact validator 必须输出 `PASS verify_project timer category empty state action contracts`，并由 `negative_timer_category_empty_state_marker_fixture` 覆盖拒绝路径。
