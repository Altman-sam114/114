...
- v1.0 的已有分类复用必须由云端检查证明两端都读取 `store.taskCategories`、排除预设、规范化去重且保持首次出现顺序，选择时只改 `category/accentHex` 草稿，代表色取首个同分类任务、session-only 时保留当前颜色，并具备非颜色选中指示、selected trait、VoiceOver/Voice Control 和 iOS 44pt 点击区。目标 marker 为 `Existing category reuse contracts verified.`，validator 目标结果为 `PASS verify_project existing category reuse contracts`；当前均为 `pending`。
- v1.0 的 run metadata 必须由云端成功 fixture、文件安全、参数矩阵、十个字段负向和 marker 缺失 fixture 证明；每个字段负例应保持三项 archive 与八项 artifact metadata PASS，确认失败只来自目标 run 字段。目标 marker 为 `CI workflow run API metadata contracts verified.`，validator 目标结果为 `PASS verify_project ci workflow run API metadata contracts`；当前未运行本地测试，尚无 commit/run/artifact/PASS 证据。
