...
- v0.99 人工硬性要求不运行任何本地测试或检查；提交并推送 `origin/main` 后，仅由 GitHub Actions 执行静态检查、`bash scripts/verify_project.sh`、Mac core/snapshot、Mac build、iOS generic build 和 artifact 生成，再由 Agent C 对自身最新 run 的原始 API JSON、原始 ZIP、manifest、marker/PASS 与 build 结果完成验收。云端结果产生前不得填写或推断 commit/run/artifact 结论。
