...
- `FocusStore` 从 `sessions` 计算今日专注、7 日趋势、分类投入时长/占比/排行位置/专注次数、最近记录和工作压力。
