...
- `scripts/validate_ci_artifact.rb` 是 Agent C 下载结果包后的结构化复判脚本，覆盖 manifest 身份和路径字段、run context 身份与 artifact 名称、artifact index 身份、必需路径与本地文件/目录非空状态、index totals 与 entries 聚合一致性、JUnit testcase、failure summary 日志入口、`verify_project.log` 分类可访问 contract marker、主日志成功标记和快照 manifest。
...
8. Agent C 可运行 `scripts/validate_ci_artifact.rb` 辅助核对结果包结构、run context、artifact 名称、分类可访问 contract marker 和关键成功标记。
