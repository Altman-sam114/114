...
- `TaskCategoryPreset`：常用分类的非持久化预设，提供标题、代表色和图标；真实分类仍保存为 `FocusTask.category`。
...
3. `FocusStore` 清洗空白分类、合并常用分类和已有分类为 `taskCategories`，供 iOS/macOS 筛选 UI 使用。
4. `FocusStore` 保存任务，并在 `autoGeneratePomodoroPlan` 启用时调用 `generatePomodoroPlanFromSchedule()`。
5. 计划按未完成任务、截止时间、剩余轮次和休息规则生成 `PomodoroPlanItem`。
6. 用户可从计划项直接开始专注，`TimerEngine.startPlanItem(_:)` 会标记计划开始并启动计时。
7. 循环任务完成后，`FocusStore.createNextRecurrenceIfNeeded(from:)` 创建下一周期任务。
8. 外部日历同步通过 `upsertExternalTask(...)` 合并到任务列表。
...
- 日程页：日历/待办、分类筛选、分类快选、任务编辑、计划生成、日历同步。
- 统计页：Pro 预览、购买、报表和分析。
...
- `AppModels.swift` 定义跨端模型契约和非持久化分类预设。
...
6. workflow 上传未加密结果包，包含 `ci-artifact-manifest.json`、`ci-failure-summary.md`、`junit.xml`、Mac/iOS build 日志、`verify_project.log`、Mac/iOS `.xcresult` 和 Mac 快照。
