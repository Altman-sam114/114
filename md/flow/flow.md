...
- `FocusSession`：已记录的专注/休息会话，用于统计和报表；`category` 会在统计最近记录中显示为分类 badge，并进入整行可访问标签与 Voice Control input labels；完成的专注会话也会按分类聚合成分类投入的时长、占比和专注次数。
...
- `FocusStore` 从 `sessions` 计算今日专注、7 日趋势、分类投入时长/占比/专注次数、最近记录和工作压力。
