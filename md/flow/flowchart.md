...
flowchart TD
  ...
  V --> CAT["TaskCategoryPreset / TaskCategoryFilterOption<br/>常用分类快选、分类计数、筛选排序、计时页筛选、新建预填、筛选摘要/快捷新增/清除"]
  CAT --> V
...
```mermaid
flowchart TD
  A["用户新增/编辑任务<br/>或系统日历同步事件"] --> P0["分类 UI<br/>计时页当前待办筛选<br/>常用分类快选、手写分类<br/>筛选摘要新增/清除<br/>筛选联动新建预填<br/>Mac 快速新增已预填提示"]
  ...
  C2 --> C3["选中分类摘要/预填提示<br/>当前数量、iOS 新增此分类、一键清除、Mac 已预填提示、空态提示"]
...
flowchart TD
  ...
  CI --> ART["未加密 CI 结果包<br/>manifest、artifact index、failure summary/错误摘录、JUnit、Mac/iOS 日志、Mac/iOS xcresult、快照、快照 manifest"]
  ART --> C["Agent C<br/>gh auth login<br/>下载 artifact 到 /private/tmp/chronofocus-c-review-run_id<br/>核对 artifact index 和快照 manifest"]
...
flowchart TD
  ...
  CI --> ART["最新未加密 artifact<br/>manifest、artifact index、JUnit、failure summary/错误摘录、日志、xcresult、快照 manifest、项目产物"]
