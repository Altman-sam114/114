...
读图说明：这张图从“用户或系统输入”开始，看数据如何进入共享状态，再由计时引擎和平台服务输出到 UI、通知、Live Activity、持久化和测试脚本。重点看 `FocusStore` 与 `TimerEngine` 的职责边界；当前任务选择行分类语义、分类筛选反选清除、统计分类投入次数/排行上下文、统计最近记录分类上下文、统计计划回顾分类语义和待办保存/取消按钮分类语义都有独立 `verify_project` marker 和 artifact validator 复判。
...
flowchart TD
  ...
  S --> C["统计和计划计算<br/>7 日趋势、分类投入时长/次数/排行、分类列表、工作压力、PomodoroPlanItem"]
...
flowchart TD
  ...
  K --> M
  M --> N["AnalyticsView / MacAnalyticsDetailView<br/>普通预览或 Pro 完整报表<br/>分类投入排行/次数语义<br/>iOS计划回顾分类badge和语音语义"]
```
