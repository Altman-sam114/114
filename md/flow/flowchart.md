...
flowchart TD
  ...
  CI --> ART["未加密 CI 结果包<br/>manifest元数据/project reports、artifact index version、精确清单和本地元数据复算、额外artifact拒绝、run context、failure summary 身份/outcome/错误摘录、JUnit 元数据/outcome、static-checks marker、Xcode 版本、verify_project 分类摘要动作/分类可访问/日程任务操作/计划开始/Mac计划分类/计划面板操作 marker、Mac/iOS 日志、Mac/iOS xcresult、快照、快照 manifest"]
  ART --> C["Agent C<br/>gh auth login<br/>下载 artifact 到 /private/tmp/chronofocus-c-review-run_id<br/>核对 artifact index、index totals、run context、artifact 名称、计划面板操作 marker 和快照 manifest"]
