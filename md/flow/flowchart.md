...
flowchart TD
  ...
  V --> CAT["TaskCategoryPreset / TaskCategoryFilterOption<br/>常用分类快选、分类计数、筛选排序、计时页筛选、新建预填、筛选摘要/清除"]
  CAT --> V
...
```mermaid
flowchart TD
  A["用户新增/编辑任务<br/>或系统日历同步事件"] --> P0["分类 UI<br/>计时页当前待办筛选<br/>常用分类快选、手写分类<br/>筛选摘要/清除<br/>筛选联动新建预填"]
